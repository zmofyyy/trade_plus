...
bars = loader.load_daily_bars(code='605068', market='sh', start_date=START_DATE, end_date=END_DATE)
print(f'Loaded {len(bars)} bars for 605068')
...
if trades:
    ...
    if pnls:
        ...
        avg_loss = abs(sum(p['pnl'] for p in pnls if p['pnl'] < 0) / len([p for p in pnls if p['pnl'] < 0])) if [p for p in pnls if p['pnl'] < 0] else 0
